    ...
    public static class MessageBox
    {
        ...
        public static async Task<MessageBoxResult> ShowAsync(string messageBoxText,
            ...
            if (button.HasFlag(MessageBoxButton.Ok)) {
                md.Commands.Add(new UICommand(ResourceLoader.GetForCurrentView().GetString("textOK"),
                    cmd => result = MessageBoxResult.Ok));
            }
            if (button.HasFlag(MessageBoxButton.Yes)) {
                md.Commands.Add(new UICommand(ResourceLoader.GetForCurrentView().GetString("textOui"),
                    cmd => result = MessageBoxResult.Yes));
            }
            if (button.HasFlag(MessageBoxButton.No)) {
                md.Commands.Add(new UICommand(ResourceLoader.GetForCurrentView().GetString("textNon"),
                    cmd => result = MessageBoxResult.No));
            }
            if (button.HasFlag(MessageBoxButton.Cancel)) {
                md.Commands.Add(new UICommand(ResourceLoader.GetForCurrentView().GetString("textAnnuler"),
